...
# Initailize session state vaiable
            ...
            with cols[i % 2]:
                ...
                with col1:
                    if st.button(f"Remove", key=f"remove_{i}", use_container_width=True):
                        if remove_book(i):
                            st.rerun()  # Ensures the UI updates after removing a book
                with col2:
                    ...
                    if st.button(status_label, key=f"status_{i}", use_container_width=True):
                        ...
                        save_library()
                        st.rerun()  # Ensures the UI updates after changing the read status
...
st.markdown("---")
st.markdown("Copyright @ 2025 Waqar Ali Personal Library Manager📚", unsafe_allow_html=True)
